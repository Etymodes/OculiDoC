...
M3D12D 已完成连续图片选择任务，并集中修正临床任务切换、设备检查和资料转移流程。
...
- 内置统一尺寸图片库与连续双图选择题；答对后在图片上显示绿色勾，按空格或 Enter 进入下一题；
- 左右/上下二分问答可从常用问题库勾选若干题连续运行，答错重试、答对后手动进入下一题；
...
- 患者管理可一键导出/录入版本化 JSON 基本资料；录入不覆盖同编号患者，也不混入实验记录；
