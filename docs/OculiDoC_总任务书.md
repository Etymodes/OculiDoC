...
> 当前开发基线：`feature/gaze-tasks-mvp`，M3D12F（本说明与实现同提交更新）。
...
项目当前已经完成真实 Tobii Eye Tracker 5 原生 Stream Engine 采样、追踪球、左右/上下二分问答、屏幕打字、图片选择、随指令注视、患者与实验会话持久化、Windows 打包及工程验收；下一阶段先完成统一实验模板 demo，再进入第二轮完善。
...
当前下一提交应聚焦 M3D12G 统一实验模板 demo；不在同一提交中提前重构 M3D13 统一运行时协议。

建议提交：

```text
Add reusable gaze experiment templates
```
...
M3D12G 完成后回到 M3D11A 开始第二轮完善，并继续按既定顺序轮询。
