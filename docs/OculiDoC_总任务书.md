...
> 当前开发基线：`feature/gaze-tasks-mvp`，M3D12C（本说明与实现同提交更新）。
...
当前下一提交应聚焦 M3D12D 图片选择 demo，并接入统一尺寸、分类和风格明确的默认备选图片；不在同一提交中提前加入随指令注视。

建议提交：

```text
Add image-choice gaze task and default stimulus library
```

预期修改：

```text
src/oculidoc/tasks/image_choice.py
src/oculidoc/ui/main_window.py
src/oculidoc/modules/registry.py
src/oculidoc/task_configs.py
tests/test_image_choice.py
README.md
docs/OculiDoC_总任务书.md
```

M3D12D 完成后立即进入 M3D12E，实现可配置目标样式和大字指令的随指令注视。
