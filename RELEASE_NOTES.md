...
## v0.1.3 开发候选：独立神经信号范式

本分支把 OculiDoC 从单一眼动平台扩展为多输入范式架构，同时保持 v0.1.2 已验收的患者、
眼动任务、会话历史和报告行为不变。

- 主页新增患者级“输入范式”多选：眼动、SSVEP、运动想象、被动 EEG；P300 仅显示为
  禁用的未来扩展位。
- 新增设备无关 `EEGSampleBlock`、患者长期信号档案、不可变会话快照和任务能力矩阵。
- 新增 Mylian 外部本地桥兼容边界、标准本地 JSON 桥、标准 NPZ 回放和 Beta00 工程模拟。
- 新增 CCA、FBCCA、TRCA、eTRCA、校准模型、评估与动态 SSVEP 刺激配置。
- 新增 EEG 质量、SSVEP 频扫/单目标/二目标/四目标/验证和 MI 独立协议。
- 信号报告记录设备、通道、算法、参数、频率、得分、置信度、拒绝与模型信息；模拟来源
  只能生成工程报告，不能进入真实患者会话或眼动纵向趋势。
- Mylian 私有字段仅停留在兼容层；仓库不捆绑厂商 DLL、SDK、许可、日志或患者数据。

此开发候选的自动化验证不等同于真实 Mylian 设备或 Augusta 床旁验收。真实硬件仍需在
目标电脑上核对外部桥、运行时/符号、驱动、许可、通道、采样率、刷新率和同步方式。
眼动、SSVEP、被动 EEG 与运动想象在 v0.1.3 分别运行、分别报告，不融合为控制指令。
