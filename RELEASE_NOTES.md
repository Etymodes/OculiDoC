...
- 新增 SSVEP“是/否”二选一任务闭环：刺激、原始数据、质量门禁、FBCCA/TRCA 识别、可见
  反馈、拒绝重试与结构化任务结果；不下发外部设备动作。
- 新增 Tieying/JustSsvep 本机 `12991` WebSocket 和历史 CSV 兼容输入。实时桥不直接占用
  `COM4`；未获证据的串口帧与 µV 换算不会被猜测。
- 预留“七鑫易维 aSee（本机 SDK 桥）”眼动源：只连接回环地址，区分桌面眼动条与穿戴式
  眼动眼镜；眼镜场景坐标在完成当前屏幕映射前不会进入既有眼动任务。
- 每试次同时保存无损 NPZ、规则矩形 CSV，并在 manifest 记录产物 SHA-256。
- 新增有标签频扫的 guarded TRCA 滚动适配：至少 4 轮、时间后置 holdout、准确率/覆盖率
  门禁和既有模型退化保护；无标签自预测永不用于训练。
- 修复深层患者会话目录在 Windows `MAX_PATH` 下创建原子临时文件失败的问题。
