...
def test_three_hits_raise_level_and_two_valid_misses_lower_it() -> None:
    ...
    for _ in range(3):
        model.record(
            model.next_target(), acquired=True, valid_sample_ratio=0.9, response_ms=500
        )
    ...
    for _ in range(2):
        model.record(
            model.next_target(), acquired=False, valid_sample_ratio=0.9, response_ms=None
        )
...
def test_invalid_tracking_never_penalizes_patient_performance() -> None:
    model = StarlightAdaptiveModel(
        StarlightRouteConfig(initial_level=4, randomization_seed=11)
    )
...
def test_edge_probe_success_expands_and_miss_tightens_reachable_region() -> None:
    ...
    for _ in range(2):
        model.record(
            model.next_target(), acquired=True, valid_sample_ratio=1.0, response_ms=400
        )
...
def test_seed_reproduces_route_and_higher_level_shrinks_star() -> None:
    ...
    for _ in range(3):
        first.record(
            first.next_target(), acquired=True, valid_sample_ratio=1.0, response_ms=300
        )
